...
export async function agentRoutes(app: FastifyInstance) {
  ...
  // POST /api/agents/:id/rotate-token — Rotate bearer token
  app.post("/api/agents/:id/rotate-token", async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const projectId = request.projectId;
    if (!projectId) {
      return reply.code(400).send({ error: "validation_error", message: "projectId is required" });
    }
    ...
    try {
      const { agent, rawToken } = await app.agentService.rotateAgentToken(
        request.params.id,
        projectId,
      );
