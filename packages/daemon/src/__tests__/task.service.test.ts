...
describe("TaskService", () => {
  ...
  describe("list", () => {
    it("filters by project", async () => {
      ...
      await service.create({ title: "B", projectId, taskType: "quick" });
    ...
    it("filters by task type", async () => {
      ...
      await service.create({ title: "Brainstorm", projectId, taskType: "brainstorm" });

      const result = await service.list({ projectId, taskType: "brainstorm" });
      expect(result).toHaveLength(1);
      expect(result[0].title).toBe("Brainstorm");
    });
  ...
  describe("convertBrainstorm", () => {
    ...
    it("rejects converting non-brainstorm task", async () => {
      ...
      await expect(
        service.convertBrainstorm(task.id, "quick")
